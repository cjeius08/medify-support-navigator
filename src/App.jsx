...
export default function App() {
  ...
  const beginEditReport = (item) => { setEditingReport(item); setEditStart(localDateTime(item.start)); setEditStop(localDateTime(item.stop)); };
  const saveEditedReport = async () => {
    const nextStart = new Date(editStart).getTime();
    const nextStop = new Date(editStop).getTime();
    if (!Number.isFinite(nextStart) || !Number.isFinite(nextStop) || nextStop < nextStart) { setToast("Please enter valid times. End time must be after start time."); return; }
    const nextSeconds = Math.floor((nextStop - nextStart) / 1000);
  ...
  return <main className="app">
    ...
    {view === "email" && <section className="page"><div className="intro"><p className="eyebrow">Email workspace</p><h1>Email notes</h1><p>Use this separate template for email cases. These fields do not appear in Call Notes or call reports.</p></div><section className="card"><div className="heading"><div><p className="eyebrow">Copy-ready email case note</p><h2>Email ticket information</h2></div><span className="tag">Email notes</span></div><div className="fields">{Object.entries(emailFields).map(([label, value]) => <label className="field" key={label}><span>{label}</span>{label === "Issue" || label === "Resolution" ? <textarea rows="4" value={value} placeholder={label === "Issue" ? "What does the customer need help with?" : "What was explained or completed?"} onChange={(e) => updateEmail(label, e.target.value)} /> : <input value={value} onChange={(e) => updateEmail(label, e.target.value)} />}</label>)}</div></section><section className="card"><div className="heading"><div><p className="eyebrow">Copy-ready note</p><h2>Email note preview</h2></div><button className="secondary-button" onClick={() => navigator.clipboard?.writeText(Object.entries(emailFields).map(([label, value]) => `${label}: ${value || "Not provided"}`).join("\n")).then(() => setToast("Email notes copied to clipboard")).catch(() => setToast("Copy was blocked. Please select the text manually."))}>Copy email notes</button></div><pre>{Object.entries(emailFields).map(([label, value]) => `${label}: ${value || "Not provided"}`).join("\n")}</pre></section></section>}
    ...
    {view === "reports" && <section className="page reports-page"><div className="intro"><p className="eyebrow">Saved on this device</p><h1>Call reports</h1><p>Review completed calls by day, week, month, quarter, or year. Clear the history whenever you need to.</p></div><div className="report-toolbar"><div className="report-tabs">{reportTypes.map((type) => <button key={type} className={reportType === type ? "active" : ""} onClick={() => setReportType(type)}>{type}</button>)}</div><button className="clear-button" disabled={!reports.length} onClick={clearHistory}>Clear all report history</button></div>{groupedReports.length ? <div className="report-groups">{groupedReports.map(([key, items]) => { const total = items.reduce((sum, item) => sum + item.seconds, 0); const average = Math.round(total / items.length); return <section className="card group-card" key={key}><div className="heading"><div><p className="eyebrow">{reportType} report</p><h2>{periodLabel(key, reportType)}</h2></div><div className="metrics"><span><b>{items.length}</b> calls</span><span><b>{elapsed(total)}</b> total</span><span><b>{elapsed(average)}</b> avg / call</span></div></div><div className="call-list">{items.map((item) => <details key={item.id}><summary><span><strong>{item.fields["Spoke With"] || "Unnamed caller"}</strong><small>{when(item.stop)}</small></span><b>{elapsed(item.seconds)}</b></summary><div className="call-actions"><button className="secondary-button" onClick={() => beginEditReport(item)}>Edit call time</button></div><dl className="rows">{Object.entries(item.fields).map(([label, value]) => <div key={label}><dt>{label}</dt><dd className={value ? "" : "empty"}>{value || "Not provided"}</dd></div>)}</dl></details>)}</div></section>; })}</div> : <section className="empty-state"><h2>No saved reports yet</h2><p>Start a call and press <strong>Stop</strong> when it ends. The report is saved automatically.</p></section>}{editingReport && <div className="edit-overlay"><section className="card edit-dialog" role="dialog" aria-modal="true" aria-labelledby="edit-call-title"><div className="heading"><div><p className="eyebrow">Correct saved report</p><h2 id="edit-call-title">Edit call time</h2></div><button className="text-button" onClick={() => setEditingReport(null)}>Close</button></div><p className="edit-help">Adjust the start or end time if you pressed Stop late. The duration and reports will update automatically.</p><div className="fields"><label className="field"><span>Call started</span><input type="datetime-local" value={editStart} onChange={(event) => setEditStart(event.target.value)} /></label><label className="field"><span>Call ended</span><input type="datetime-local" value={editStop} onChange={(event) => setEditStop(event.target.value)} /></label></div><div className="bottom"><button className="secondary-button" onClick={() => setEditingReport(null)}>Cancel</button><button className="report-button" onClick={saveEditedReport}>Save corrected time</button></div></section></div>}</section>}
