...
export default function App() {
  ...
  const clearHistory = () => { if (!reports.length) return; if (window.confirm("Clear all saved call reports? A save point will be created so you can undo this.")) { localStorage.setItem("medify-cleared-call-reports", JSON.stringify(reports)); setClearedReports(reports); setReports([]); setToast("Report history cleared. You can undo it below."); } };
  const undoClearHistory = () => { if (!clearedReports.length) return; setReports(clearedReports); setClearedReports([]); localStorage.removeItem("medify-cleared-call-reports"); setToast("Cleared reports restored."); };
